...
chars = []
chars+=['4f', '4d', '41', '49', '44', '4f', '4d', '41', '49', '44', '01', '02', '03', '04', '05', '06']
...
def whatop(char):
	...
	for insn in md.disasm(char, 0x00):
		return "%s : \t%s\t%s " %(char, insn.mnemonic, insn.op_str)
...
def convert(payload):
	...
	for char in payload:
		...
		if hexchar not in chars:
			fail += char
			print "FAIL " + hexchar
			# md = Cs(CS_ARCH_X86, CS_MODE_32)
			# for insn in md.disasm(char, 0x1000):
...
# Shows all characters you cannot use(assuming no payload conversion) in hex string format
bca = 'badchars = "'
...
# Shows all characters you can use in hex string format
gca = 'goodchars = "'
