...
def BadChars(original,result):
    for old,new in zip(original,result):
        if old == new:
            ...
            # md = Cs(CS_ARCH_X86, CS_MODE_32)
            # for insn in md.disasm(old.encode('hex'), 0x1000):
            ...
            print "exploit = exploit.replace(\\x%s,\\x%s)" % (old.encode('hex'),new.encode('hex'))
...
def printop(char):
    ...
    for insn in md.disasm(char, 0x1000):
        print "%s : \t%s\t%s " %(char.encode('hex'), insn.mnemonic, insn.op_str)
